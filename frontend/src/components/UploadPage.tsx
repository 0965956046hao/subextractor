    ...
    <div className="space-y-6">
      <div className="double-bezel">
        <div className="double-bezel-inner">
          ...
          >
            <input ref={inputRef} type="file" accept="video/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFile(f); }} className="hidden" />
            ...
            {loading ? (
              <div className="flex flex-col items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-ink-muted mt-1.5">Uploading to server...</p>
                </div>
                <div className="w-full max-w-xs space-y-2">
                  <div className="h-1.5 rounded-full bg-black/[0.06] overflow-hidden">
                    <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)]" style={{ width: `${Math.max(progress, 2)}%` }} />
                  </div>
                  <p className="text-xs font-mono text-ink-light text-center">{progress}%</p>
                </div>
                {error && (
              ...
              <div className="flex flex-col items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-sm text-ink-muted mt-1.5">
                    or <span className="text-blue-600 underline underline-offset-2 decoration-blue-500/30">browse files</span> &mdash; MP4, MOV, AVI, MKV, WebM
                  </p>
                </div>
                {error && (
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-500/10 ring-1 ring-red-500/15 text-xs text-red-600/80">
      ...
      <div className="flex items-center justify-center gap-2 flex-wrap">
        <span className="text-[10px] uppercase tracking-wider text-ink-light mr-1">Supported</span>
        {FORMATS.map((ext) => (
