...
//     { name: string; display_name: string; size_bytes: number }[]
//   >([]);
...
//     setGenError("");
//     try {
//       const res = await fetch(`/api/context/${videoId}/generate`, {
//         method: "POST",
//       });
//       const data = await res.json();
//       if (data.job_id) {
...
//         `/api/gemini/files?video_id=${encodeURIComponent(videoId)}`,
//       );
//       const data = await res.json();
...
//     try {
//       await fetch(`/api/gemini/files/${encodeURIComponent(name)}`, {
//         method: "DELETE",
//       });
...
//             <div
//               className={`w-8 h-8 rounded-xl flex items-center justify-center ${context ? "bg-violet-500/10 ring-1 ring-violet-500/20" : "bg-black/[0.03]"}`}
//             >
//               <IconSparkle
//                 className={`w-4 h-4 ${context ? "text-violet-500" : "text-ink-light"}`}
//               />
//             </div>
//             <div>
...
//                 {context
//                   ? "Gemini Vision đã phân tích video"
//                   : "Chưa có ngữ cảnh"}
//               </p>
//             </div>
//           </div>
...
//               Chưa có ngữ cảnh video
//             </p>
//             <p className="text-[12px] text-ink-light max-w-sm mx-auto">
//               Nhấn &quot;Phân tích video&quot; để Gemini Vision phân tích các
//               ảnh snapshot và tạo mô tả ngữ cảnh, giúp dịch phụ đề chính xác
//               hơn.
//             </p>
//           </div>
//         ) : null}
...
//               Khi bạn dùng tính năng{" "}
//               <span className="font-medium text-ink/70">Dịch (Gemini)</span> ở
//               tab Timeline, ngữ cảnh này sẽ được gửi kèm để Gemini hiểu rõ bối
//               cảnh video và dịch từ ngữ sát nghĩa hơn.
//             </p>
//           </div>
//         )}
...
//           >
//             <svg
//               className={`w-3 h-3 transition-transform duration-400 ${showFileStore ? "rotate-90" : ""}`}
//               viewBox="0 0 24 24"
//               fill="none"
//               stroke="currentColor"
//               strokeWidth={1.5}
...
//               <span className="px-1.5 py-0.5 rounded-full bg-amber-500/15 text-[9px]">
//                 {geminiFiles.length}
//               </span>
//             )}
//           </button>
...
//           {showFileStore && (
//             <div
//               className="mt-2 rounded-2xl bg-amber-500/[0.02] ring-1 ring-amber-500/[0.08] overflow-hidden"
...
//                     Không có file nào trong Gemini File Store
//                   </p>
//                 </div>
//               ) : (
...
//                     <div
//                       key={f.name}
//                       className="flex items-center justify-between px-4 py-2.5 border-b border-amber-500/[0.06] last:border-b-0 hover:bg-amber-500/[0.03] transition-colors"
//                     >
//                       <div className="min-w-0 flex-1 mr-3">
//                         <p className="text-[12px] font-medium text-ink/80 truncate">
